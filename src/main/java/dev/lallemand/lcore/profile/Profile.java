...
import dev.lallemand.lcore.profile.conversation.ProfileConversations;
...
import dev.lallemand.lcore.profile.option.ProfileOptions;
...
import dev.lallemand.lcore.profile.staff.ProfileStaffOptions;
import dev.lallemand.lcore.profile.staff.events.PlayerVisibilityChangeEvent;
import dev.lallemand.lcore.profile.staff.events.StaffModeUpdateEvent;
import lombok.Data;
...
import dev.lallemand.lcore.util.hotbar.Hotbar;
import dev.lallemand.lcore.util.item.ItemBuilder;
import dev.lallemand.lcore.util.player.NameTagHandler;
...
public class Profile {
    ...
    @Getter private static MongoCollection<Document> collection = lCore.get().getMongoDatabase().getCollection("profiles");
    ...
    private List<dev.lallemand.lcore.profile.notes.Note> notes;
    ...
    public static Profile getByUsername(String username) {
        ...
        UUID uuid = lCore.get().getRedisCache().getUuid(username);
    ...
    public boolean isAuthenticated() {
        return !lCore.get().getMainConfig().getBoolean("SETTINGS.2FA_ENABLED") || authenticated;
    }
    ...
    public void toggleStaffMode() {
        if (getPlayer().isOnline()) {
            ...
            if (this.getStaffOptions().isStaffModeEnabled()) {
                ...
                if (lCore.get().getMainConfig().getBoolean("COMMON.COLOR-TAG-ENABLED")) {
                    for (Player online : Bukkit.getOnlinePlayers()) {
                        NameTagHandler.addToTeam(online, getPlayer(), CC.translate("&7[S] " + lCoreAPI.getColorOfPlayer(getPlayer())), false);
                    }
                }
            } else {
                PlayerUtil.reset(getPlayer(), true);
    ...
    public void checkGrants() {
        ...
        if (activeGrant != null) {
            if (!activeGrant.isRemoved() && activeGrant.hasExpired()) {
                ...
                activeGrant.setRemovedReason("Grant expired");
    ...
    public boolean removeNote(int id) {
        dev.lallemand.lcore.profile.notes.Note note = notes.stream().filter(note1 -> note1.getId() == id).findFirst().orElse(null);
        return notes.remove(note);
    }

    public dev.lallemand.lcore.profile.notes.Note getNote(int id) {
    ...
    public void setupBukkitPlayer(Player player) {
        ...
        for (PermissionAttachmentInfo attachmentInfo : player.getEffectivePermissions()) {
            if (attachmentInfo.getAttachment() == null || attachmentInfo.getAttachment().getPlugin() == null ||
                !attachmentInfo.getAttachment().getPlugin().equals(lCore.get())) {
                continue;
            }
        ...
        PermissionAttachment attachment = player.addAttachment(lCore.get());
        ...
        String coloredName = lCoreAPI.getColoredName(player);
    ...
    public void load() {
        ...
        if (document != null) {
            ...
            ipAddresses = lCore.GSON.fromJson(document.getString("ipAddresses"), lCore.LIST_STRING_TYPE);
            ...
            if (document.getString("permissions") != null) {
                permissions = lCore.GSON.fromJson(document.getString("permissions"), lCore.LIST_STRING_TYPE);
            }
            ...
            if (document.getString("notes") != null) {
                ...
                for (JsonElement noteData : notesArray) {
                    ...
                    dev.lallemand.lcore.profile.notes.Note note = dev.lallemand.lcore.profile.notes.Note.DESERIALIZER.deserialize(noteData.getAsJsonObject());
    ...
    public void save() {
        ...
        document.put("ipAddresses", lCore.GSON.toJson(ipAddresses, lCore.LIST_STRING_TYPE));

        document.append("alts", this.alts);


        document.put("permissions", lCore.GSON.toJson(permissions, lCore.LIST_STRING_TYPE));
        ...
        for (UUID uuid : ignoreList) {
            ignoreArray.add(uuid.toString());
        }

        document.put("ignorelist", ignoreArray.toString());

        JsonArray notesArray = new JsonArray();
        for (dev.lallemand.lcore.profile.notes.Note note : notes) {
